...
const mockDocuments = [
...

  {
    id: 3,
    type: "Roadmap",
    title: "Career Roadmap for Data Science",
    date: "July 20, 2025",
    icon: <Milestone />,
  },

  {
    id: 4,
    type: "Resume",
    title: "Resume for Product Manager",
    date: "July 19, 2025",
    icon: <FileText />,
  },
...
const MyDocumentsPage = () => {
  ...
  const handleDelete = async (docId, docType) => {
    ...
    const collectionName =
      docType === "Resume" ? "resumeAnalyses" : "coverLetters";
  ...
  return (
    <Box>
      ...
      <Typography color="text.secondary" sx={{ mb: 3 }}>
        Access all your generated resumes, cover letters, and career roadmaps
        here.
      </Typography>
      ...
      >
        <ButtonGroup
          variant="outlined"
          aria-label="document filter button group"
        >
          <Button
          ...
          <Button
            ...
            variant={filter === "Resume" ? "contained" : "outlined"}
          >
            Resumes
          </Button>
          <Button
            onClick={() => setFilter("Cover Letter")}
            variant={filter === "Cover Letter" ? "contained" : "outlined"}
          >
            Cover Letters
          </Button>
          <Button
            onClick={() => setFilter("Roadmap")}
            variant={filter === "Roadmap" ? "contained" : "outlined"}
          >
            Roadmaps
          </Button>
        </ButtonGroup>
      ...
      {view === "list" ? (
        <div className="vstack gap-2">
          {filteredDocuments.map((doc) => (
            <DocumentListItem key={doc.id} {...doc} onDelete={handleDelete} />
          ))}
        </div>
      ) : (
        <div className="row g-3">
          {filteredDocuments.map((doc) => (
            <div key={doc.id} className="col-12 col-sm-6 col-lg-4 d-flex">
              <DocumentCard {...doc} onDelete={handleDelete} />
            </div>
          ))}
        </div>
      )}
