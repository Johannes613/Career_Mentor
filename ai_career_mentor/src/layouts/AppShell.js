...
import { Box, AppBar, Toolbar, Drawer, List, ListItem, ListItemButton, ListItemIcon, ListItemText, Typography, IconButton, Avatar, useTheme, useMediaQuery } from '@mui/material';
...
const AppShell = ({ children, navItems, activePageId, onNavItemClick }) => {
    ...
    return (
        <Box sx={{ display: 'flex' }}>
                ...
                <List sx={{ px: 2, py:1, width: '100%' }}>
                    {navItems.map((item) => (
                        <ListItem key={item.text} disablePadding>
                            ...
                            >
                                <ListItemIcon sx={{ minWidth: 0, mr: isCollapsed ? 0 : 3, justifyContent: 'center', color: 'inherit' }}>
                                    {item.icon}
                                </ListItemIcon>
                                {!isCollapsed && (
            ...
            <Box component="main" sx={{ flexGrow: 1, backgroundColor: 'background.default', height: '100vh', overflow: 'auto' }}>
                <AppBar
                    position="static"
                    elevation={0}
                    sx={{
                ...
                >
                    <Toolbar>
                        <IconButton color="primary" onClick={handleDrawerToggle} sx={{ mr: 2 }}>
                ...
                {/* Chatbot appears on all pages that use AppShell */}
                <Chatbot />
            </Box>
        </Box>
    );
